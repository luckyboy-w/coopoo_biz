					...
					<el-table
						...
						<el-table-column prop="id" label="操作" width="200px"  v-if="showOp">
							<template slot-scope="scope">
								...
								<!--isSale;-->
								<el-button v-if="scope.row.verifyStatus == 20" type="text" size="small" @click.native.prevent="addStock(scope.row)">补货 </el-button>
								<el-button v-if="scope.row.verifyStatus == 5" type="text" size="small" @click="modityGood(scope.row,'verifyStatus','10')" >提交审核</el-button>
								<el-button v-if="scope.row.verifyStatus == 30 || scope.row.verifyStatus == 5 || scope.row.isSale == 3 " type="text" size="small" @click="addOrEdit('edit',scope.$index, tableData)" >编辑</el-button>
								...
								<el-button v-if="scope.row.verifyStatus == 20 && scope.row.isSale == 1 " type="text" size="small" @click="offLine(scope.row,'3')">下架</el-button>
							</template>
						</el-table-column>
					</el-table>
				</div>
		...
		<el-dialog title="补货"
			...
			<el-row :gutter="20" style="line-height:40px;font-size:12px">
				<el-table
					...
					<el-table-column prop="supplyPrice" label="供货价" width="120px" ></el-table-column>
